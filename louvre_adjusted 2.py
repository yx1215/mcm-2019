...
if __name__ == '__main__':
    ...
    #         if not is_stairs[i][j - 1]:
    #             ebunch.append((i + 1, j, Louvre_weight[i][j - 1]))
    #         else:
    #             ebunch.append((i + 1, j, 0.5))
    # print('ebunch:', ebunch)
    for i in range(1, 91):
        G.add_node(i)
    ...
    #
    net = Network.Network(height=2000, width=2000, notebook=True, directed=True)
    net.from_nx(G)
    net.inherit_edge_colors_from(True)
    net.toggle_hide_edges_on_drag(True)
    net.barnes_hut()
    net.show("test.html")
    # [22, 23, 40, 46, 48, 49, 65]
    # [3, 4, 5, 20, 21, 23, 24, 26, 36, 41, 45, 47, 49, 50, 56, 57, 59, 60, 63, 64, 65, 66, 67, 72, 80, 81, 82, 83, 84, 85, 89]
